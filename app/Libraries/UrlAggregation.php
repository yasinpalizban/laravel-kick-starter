...
use App\Interfaces\UrlAggregationInterface;
...
class  UrlAggregation implements UrlAggregationInterface
{
    ...
    public function dataMap(array $dataMap): void
    {
        ...
        $this->filed = $this->removeCamelCase($this->filed);
        $this->sort = $this->removeCamelCase($this->sort);
    ...
    public function decodeQueryParam(): UrlAggregation
    {
        ...
        foreach ($this->q as $key => $value) {
            ...
            if (isset($this->pipeLine[$object->fun])) {

                $this->pipeLine[str_replace(' ', '', $object->fun)] = array_merge($this->pipeLine[$object->fun], $temp);
            } else {
    ...
    function getPipeLine(?array $defaultPipeLine = null): array
    {
        ...
        if (($this->filed != '') || isset($defaultPipeLine['select'])) {


            $this->pipeLine['select'] = !empty($this->filed) ? $this->filed : $defaultPipeLine['select'];

        } else {

            $this->pipeLine['select'] = $this->tableName . '*';
        }
        ...
        if (isset($defaultPipeLine['orWhere']) && !isset($this->pipeLine['orWhere'])) {
            $this->pipeLine['orWhere'] = $defaultPipeLine['orWhere'];

        } else if (isset($defaultPipeLine['orWhere']) && isset($this->pipeLine['orWhere'])) {
            $this->pipeLine['orWhere'] = array_merge($this->pipeLine['orWhere'], $defaultPipeLine['orWhere']);
        }
    ...
    private function removeCamelCase(string $str): string
    {
        $text = preg_replace(['/([a-z\d])([A-Z])/', '/([^_])([A-Z][a-z])/'], '$1_$2', $str);
