...
class PermissionGroupController extends ApiController
{
    ...
    public function index(Request $request, PermissionGroupService $permissionGroupService, int $permission_id = 0)
    {
        ...
        $permissionGroupFilter = new PermissionGroupFilter();
        $permissionGroupFilter->transform($request)->navigation($request);
        $data = $permissionGroupService->setNestId($permission_id)->index($permissionGroupFilter);
